...
class PluginTest(testtools.TestCase):
    ...
    @mock.patch('vif_plug_vrouter.linux_net.device_exists', return_value=True)
    def test_unplug_vrouter(self, device_exists):
        with mock.patch.object(processutils, 'execute') as execute:
            ...
            execute.assert_has_calls([
                ...
                mock.call(
                    'ip', 'link', 'delete', 'tap-xxx-yyy-zzz',
    ...
    def test_plug_vrouter(self):
        ...
        with mock.patch.object(processutils, 'execute') as execute:
            ...
            execute.assert_has_calls([
                mock.call(
                    'ip', 'tuntap', 'add', 'tap-xxx-yyy-zzz', 'mode',
                    'tap', run_as_root=True, check_exit_code=[0, 2, 254]
                ),
                mock.call(
                    'ip', 'link', 'set', 'tap-xxx-yyy-zzz', 'up',
                    run_as_root=True, check_exit_code=[0, 2, 254]
                ),
                mock.call(
                    'vrouter-port-control',
